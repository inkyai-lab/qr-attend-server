...
/**
 * @description : verifyEmployee user.
 * @param {string} employeeId : username of user.
 * @param {string} email : password of user.
 * @return {Object} : returns authentication status. {flag, data}
 */

const verifyEmployee = async (staff_id, email) => {
  try {
    let where = { 
      staff_id: staff_id, 
      email: email
    ...
    let user = await dbService.findOne(Employee,where);
    if (user) {
      return {
        flag:false,
        data:'An employee'
      ...
      return {
        flag:true,
        data:'Not an employee in this organisation'
      };
    }
  } catch (error) {
