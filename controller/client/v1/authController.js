...
const register = async (req,res) =>{
  try {
    ...
    let employeeCheck = await authService.verifyEmployee(staff_id, email);
    if (employeeCheck.flag){
      return res.badRequest({ message: employeeCheck.data });
    }
    ...
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists. Ony unique ${checkUniqueFields.field} are allowed.` });
    }

    const result = await dbService.create(User,data);
    
    // if (req.body.email){
    //   await authService.sendWelcomeNotification({
